...
class YqUser:
    ...
    def login(self):
        ...
        User login
        '''
        print('user {} login...'.format(self.name), end='', flush=True)
    ...
    def login_confirm(self):
        ...
        try:
            resp_data = get_resp_data('/app/call/service.do', data_server, req_headers, req_data_val,self.name)
    ...
    def get_work_group(self):
        ...
        resp_data = get_resp_data('/app/group/service.do', login_server, req_headers, req_data_val,self.name)
    ...
    def show_statis(self):
        ...
        resp_data = get_resp_data('/app/service.do', data_server, req_headers, req_data_val,self.name)
    ...
    def show_sys_notify(self, not_viewed=False):
        ...
        resp_data = get_resp_data('/app/user/service.do', data_server, req_headers, req_data_val,self.name)
    ...
    def sign_on(self, locate):
        ...
        req_data_val = {'city': locate['city'], 'country': locate['country'], 'groupId': '', 'imei': imei,
                        'userName': self.name, 'userId': self.id, 'msgId': 'APP_SIGN',
                        'position': locate['position'], 'province': locate['province'], 'longitude': longitude,
                        'latitude': latitude}
        ...
        for group_id, group_name in self.group_list.items():
            ...
            resp_data = get_resp_data('/app/sign/service.do', data_server, req_headers, req_data_val,self.name)
    ...
    def get_news_list(self, categoryId, max_count, not_viewed):
        ...
        while True:
            ...
            resp_data = get_resp_data('/app/newsActivity/service.do', data_server, req_headers, req_data_val,self.name)
    ...
    def share_news(self):
        ...
        for news in news_list:
            ...
            try:
                ...
                req_data_val = {'content': news['title'], 'userId': self.id, 'groupName': 'yq_crack', 'msgId': 'APP083',
                                "isOriginal": False, 'groupId': 2002647, 'type': 1, "isForward": False}
                resp_data = get_resp_data('/app/group/service.do', data_server, req_headers, req_data_val,self.name)
            except Exception as e:
                print('Error({})'.format(e), ' ')
            else:
                print('OK', ' ')
    ...
    def watch_one_video(self, video, quick):
        ...
        print('Watching video: {}  author: {} time: {}(s)'.format(video['title'], video['source'], video_time),
              flush=True)

        now = datetime.today()
        begin_time = '{}-{:0>2d}-{:0>2d} {:0>2d}:{:0>2d}:{:0>2d}'.format(now.year, now.month, now.day, now.hour,
        ...
        try:
            ...
            req_data_val = {'department': self.depart, 'msgId': 'APP008', 'objectId': video['id'],
                            'orgId': video['orgId'], 'userId': self.id}
            resp_data = get_resp_data('/app/newsActivity/service.do', data_server, req_headers, req_data_val,self.name)

            video_data = {'userId': self.id, 'newsId': str(video['id']), 'playBeginTime': begin_time,
            ...
            resp_data = get_resp_data('/app/statis/service.do', data_server, req_headers, req_data_val,self.name)

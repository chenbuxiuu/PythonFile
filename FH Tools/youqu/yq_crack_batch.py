    ...
    try:
        ...
        with open(YqUser.user_file, 'r') as f:
            for line in f.readlines():
                ...
                print(name)
                m, p = mp.split(':')
                userinfo_dict[m.strip()] = p.strip('\r\n')

        for (mobile, passwd) in userinfo_dict.items():
            user = YqUser(mobile, passwd)
            user.show_statis()
...
if __name__ == '__main__':
	...
	scheduler.add_job(main, 'cron', day_of_week='mon-fri', hour='10',minute='43',second ='0',misfire_grace_time =300)
